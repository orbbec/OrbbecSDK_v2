...
public:
     ...
     * Users who wish to set custom full-scale ranges or sample rates should refer to the product manual, as available settings vary by device model.
     *
     * @param scanSpeed The scan speed of the LiDAR (default is OB_LIDAR_SCAN_ANY, which selects the default speed).
     * @param format The video stream format (default is OB_FORMAT_ANY, which selects the default format).
     */
    void enableLiDARStream(OBLiDARScanSpeed scanSpeed = OB_LIDAR_SCAN_ANY, OBFormat format = OB_FORMAT_ANY) const {
        ob_error *error = nullptr;
        ob_config_enable_lidar_stream(impl_, scanSpeed, format, &error);
        Error::handle(&error);
    }
