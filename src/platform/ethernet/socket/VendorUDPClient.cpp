...
VendorUDPClient::VendorUDPClient(std::string address, uint16_t port, uint32_t connectTimeout, uint32_t commTimeout)
...
void VendorUDPClient::socketConnect() {
    int rst;
    socketFd_ = socket(AF_INET, SOCK_DGRAM, 0);  // ipv4, udp(Streaming)
    ...
    if(rst < 0) {
        throw libobsensor::invalid_value_exception(utils::string::to_string() << "VendorUDPClient: ioctlsocket to non-blocking mode failed! addr=" << address_
...
#endif
            throw libobsensor::invalid_value_exception(utils::string::to_string() << "VendorUDPClient: Connect to server failed! addr=" << address_
    ...
    LOG_DEBUG("UDP client socket created!, addr={0}, port={1}, socket={2}", address_, port_, socketFd_);
}
...
void VendorUDPClient::socketReconnect() {
    LOG_INFO("UDP client socket reconnecting...");
    socketClose();
    socketConnect();
}
    ...
    while(retry-- && !flushed_) {
        ...
        if(rst < 0) {
            ...
            if((rst == EAGAIN || rst == EWOULDBLOCK) && retry >= 1) {
                ...
                throw libobsensor::io_exception(utils::string::to_string() << "VendorUDPClient read data failed! socket=" << socketFd_ << ", err_code=" << rst);
            }
        }
        else {
            return rst;
